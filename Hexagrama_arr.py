...
class Numero:
    ...
    def regla1(self):
        ...
        comb_validas = np.array([c for c in combinations(val_numeros, 4) if sum(x + 1 for x in c) == 26])
    ...
    def regla3(self): #No pueden haber números repetidos.
        num_casillas = np.array([(n,c) for n in range(self.N) for c in range(self.C)])
        lista = []
        for m in num_casillas:
            n,c = m
            otras_casillas = np.array([(c1) for c1 in range(self.C) if (c1) != (c)])
            lista_o = []
    ...
    def visualizar(self, I):
        ...
        ax.set_ylim(-1.2, 1.2)
        ax.set_aspect('equal')
        plt.axis('off') #Escondemos los ejes para una mejor visualización
        
        #Hacemos las direcciones de cada casilla
        direcciones = [
            [0, 1], [-0.866, 0.5], [-0.2886, 0.5], [0.2886, 0.5], [0.866, 0.5],
            [-0.5773, 0], [0.5773, 0], [-0.866, -0.5], [-0.2886, -0.5],
            [0.2886, -0.5], [0.866, -0.5], [0, -1]
        ]

        #Usamos un diccionario que mapee cada casilla a una lista de números para permitir más de un número por casilla
        casilla_a_numeros = {c: [] for c in range(self.C)}

        for literal in I:
            if I[literal]:  #Si el literal es Verdadero
                if '-' in literal:
                    continue  #Ignora negaciones
                n, c = self.NenC.unravel(literal)
                #Añadimos el número (n+1) a la lista de la casilla c
                casilla_a_numeros[c].append(n + 1)

        for c in range(self.C):
            if casilla_a_numeros[c]:
                x, y = direcciones[c]
                #Unimos todos los números de la casilla con comas (o '\n' para salto de línea)
                #Ordenamos los números para que la visualización sea consistente
                valores_str = ", ".join(map(str, sorted(casilla_a_numeros[c])))
                
                #Ajustamos el tamaño de fuente si hay muchos valores para que quepan
                font_size = 14 if len(casilla_a_numeros[c]) <= 1 else 9
                ...
                ax.text(
                    ...
                    fontweight='bold',
                    color='#D15CC9',
                    bbox=dict(facecolor='white', alpha=1, edgecolor='none', boxstyle='circle')
                )
